...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Panggil AI dengan Model Terbaru
    ...
    const groqRes = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: "llama-3.1-8b-instant", // Model fallback jika scout tidak ada di endpoint ini
        messages: [{ role: "user", content: prompt }],
        temperature: 0.7,
      }),
    });

    if (!groqRes.ok) throw new Error(`AI API Error: ${groqRes.statusText}`);
